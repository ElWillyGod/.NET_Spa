...
namespace Logica
{
    public class logica
    {
        ...
        //##########################################NO TOCAR########################################### 
        //##########################################NO TOCAR########################################### 
        public static bool reservarServiciosv2(int nro, string fecha)
        {
            foreach (DataRow dr in Reservas.reservas.Rows)
            {
                if (!Datos.Reservas.ReservarServicio(nro, dr["Hora"].ToString(), Int32.Parse(dr["CI"].ToString()), dr["Servicio"].ToString(),
                                 Int32.Parse(dr["Costo"].ToString())))
                {
                    return false;
                }
                Reservas.LogReservarServicio(UsuarioActual, nro, dr["Hora"].ToString(), Int32.Parse(dr["CI"].ToString()),"Servicio reservado");
                Datos.Fecha.ModificarCupos(fecha, dr["Hora"].ToString(), dr["Servicio"].ToString(), false);
                Datos.Fecha.modificarCapacidad(dr["Servicio"].ToString(), fecha,false);
            }
            return true;
        }
        ...
        public static bool checkIda2()
        {
            int ci = 0;
            DateTime xd = DateTime.MinValue;
            foreach(DataRow dr in Reservas.reservas.Rows)
            {
                if(dr["Servicio"].ToString()=="Traslado Ida")
                {
                    xd = Convert.ToDateTime(dr["Hora"].ToString());
                    ci = Int32.Parse(dr["CI"].ToString());
                    foreach (DataRow dra in Reservas.reservas.Rows)
                    {
                        if (xd >= Convert.ToDateTime(dra["Hora"].ToString()) && dra["Servicio"].ToString() != "Traslado Ida" && Int32.Parse(dra["CI"].ToString()) == ci)
                        {
                            return false;
                        }
                    }
        ...
        public static bool checkVuelta2()
        {
            int ci = 0;
            DateTime xd = DateTime.MinValue;
            foreach (DataRow dr in Reservas.reservas.Rows)
            {
                if (dr["Servicio"].ToString() == "Traslado Vuelta")
                {
                    xd = Convert.ToDateTime(dr["Hora"].ToString());
                    ci = Int32.Parse(dr["CI"].ToString());
                    foreach (DataRow dra in Reservas.reservas.Rows)
                    {
                        if (xd <= Convert.ToDateTime(dra["Hora"].ToString()) && dra["Servicio"].ToString() != "Traslado Vuelta" && Int32.Parse(dra["CI"].ToString()) == ci)
                        {
                            return false;
                        }
                    }
        ...
        public static bool checkTrasladoPago(int ci)
        {
            foreach(DataRow dr in Reservas.reservas.Rows)
            {
                if ((dr["Servicio"].ToString()== "Traslado Ida" || dr["Servicio"].ToString() == "Traslado Vuelta") && ci == Int32.Parse(dr["CI"].ToString()))
                {
                    return true;
                }
            }
        ...
        public static void ValidarNumerosYGuion(KeyPressEventArgs e)
        {
        ...
        public static int ListarServiciosTicket(PrintPageEventArgs e,int total, int f,int pago)
        {
            ...
            foreach(DataRow dr in Reservas.reservas.Rows)
            {
                        x++;
                        y = f + (15 * x);
                            graphics.DrawString(dr["Servicio"].ToString(), fuente, Brushes.Black, new Point(10, y));
                            graphics.DrawString(dr["Hora"].ToString(), fuente, Brushes.Black, new Point(120, y));
                            graphics.DrawString("$"+dr["Costo"].ToString(), fuente, Brushes.Black, new Point(230, y));
            ...
            y += 15;
            graphics.DrawString("Pago:", Titulos, Brushes.Black, new Point(185, y));
            graphics.DrawString("$" + pago, fuente, Brushes.Black, new Point(225, y));
            y += 15;
            graphics.DrawString("Restante:", Titulos, Brushes.Black, new Point(170, y));
            if(total<=pago)
            {
                graphics.DrawString("$0", fuente, Brushes.Black, new Point(225, y));
                y += 15;
                graphics.DrawString("Devuelto:", Titulos, Brushes.Black, new Point(170, y));
                graphics.DrawString("$" + (pago-total), fuente, Brushes.Black, new Point(225, y));
            } 
            else
            {
                graphics.DrawString("$" + (total-pago), fuente, Brushes.Black, new Point(225, y));
            }
        ...
        public static void ImprimirTicket(PrintDocument ticket)
        {
            try
            {
                PrintPreviewDialog hola = new PrintPreviewDialog();
                hola.Document = ticket;
                hola.ShowDialog();
            }
            catch (Exception e)
            {
        ...
        public static void InformacionImprimir(PrintPageEventArgs e,int nroReserva,string FormaPago,string nombre,string apellido,int cedula,string fecha,DataGridView dgv,int total,string entrada,bool yaExiste,int pago)
        {
            try
            {
                int y = 0;
                ...
                if (!yaExiste)
                {
                    ...
                    y = ListarServiciosTicket( e, total, 365,pago);
                }
                else
                {
                    y = ListarServiciosTicket(e, total, 350,pago);
                }
        ...
        public static void ExportToPdf(DataTable dt,string address,string titulo)
        {
            Document document = new Document();
            PdfWriter writer = PdfWriter.GetInstance(document, new FileStream(address, FileMode.Create));
            document.Open();
            iTextSharp.text.Font datos = iTextSharp.text.FontFactory.GetFont(FontFactory.HELVETICA, 8);
            iTextSharp.text.Font datosHeader = iTextSharp.text.FontFactory.GetFont(FontFactory.HELVETICA, 10, new iTextSharp.text.BaseColor(255,255, 255, 255));
            iTextSharp.text.Font titleFont = FontFactory.GetFont("Arial", 25);
            iTextSharp.text.Font horaFont = FontFactory.GetFont("Arial", 10);
            Paragraph title = new Paragraph(titulo, titleFont);
            Paragraph hora = new Paragraph("Generado el: "+DateTime.Now.ToString(), horaFont);
            Paragraph persona = new Paragraph("Generado por: "+Datos.Usuarios.NApellidoUsuario(logica.UsuarioActual), horaFont);
            Paragraph pc = new Paragraph("Generado en: " + Environment.MachineName, horaFont);
            hora.Alignment = Element.ALIGN_RIGHT;
            title.Alignment = Element.ALIGN_CENTER;
            persona.Alignment = Element.ALIGN_RIGHT;
            ...
            {
                PdfPCell cell = new PdfPCell(new Phrase(c.ColumnName, datosHeader));
                cell.HorizontalAlignment = PdfPCell.ALIGN_CENTER;
                cell.BackgroundColor = new iTextSharp.text.BaseColor(0, 126, 249, 255);
                table.AddCell(cell);
            ...
            foreach (DataRow r in dt.Rows)
            {
                ...
                for(int i = 0; i < dt.Columns.Count; i++) {
                    PdfPCell cell = new PdfPCell(new Phrase(r[i].ToString(), datos));
                    cell.HorizontalAlignment = PdfPCell.ALIGN_CENTER;
                    
                    if (x% 2 == 0)
                    {
                        cell.BackgroundColor = new iTextSharp.text.BaseColor(128, 128, 128, 128);
                    }
                    table.AddCell(cell);
                } 
            }
        ...
        public static void CargarHoras(ComboBox comboApertura)
        {
            comboApertura.Items.Clear();
            DateTime hora = DateTime.MinValue;
            int x = 0;
            while(x!=48)
            {
                comboApertura.Items.Add(hora.AddMinutes(x * 30).ToString("HH:mm"));
                x++;
            }
            
        }
        public static void CargarHorarioDelSpa(ComboBox combo)
        {

            
            DateTime horaApertura = Convert.ToDateTime(Spa.gethoraApertura());
            DateTime horaCierre = Convert.ToDateTime(Spa.gethoraCierre());
            int y = horaCierre.Hour-horaApertura.Hour;
           for(int x=0;x<y;x++)
            {
                combo.Items.Add(horaApertura.AddHours(x).ToString("HH:mm"));
            }
        }
        public static void AbrirPortaVasos()
        {
            int ret = mciSendString("set cdaudio door open", null, 0, IntPtr.Zero);
        }
        public static void CerrarPortaVasos()
        {
