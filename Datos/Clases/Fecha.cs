...
namespace Datos
{
    public class Fecha
    {
        ...
        public static bool AgregarHoras(string servicio, string fecha)
        {
            ...
            if (!CheckFecha(fecha, servicio))
            {
                try
                {
                    ...
                    for (int i = 0; i < j; i++)
                    {
                        string Query = "insert into Pollux.fecha_disponible(Servicio,Fecha,Hora,Cupos) values(@servicio,@fecha,@hora,@cupos);";
        ...
        public static bool CheckFecha(string fecha, string servicio)
        {
            try
            {
                string Query = "SELECT COUNT(*) FROM Pollux.fecha_disponible WHERE Fecha= @fecha and Servicio=@servicio;";
                MySqlCommand comando = new MySqlCommand(Query, ConexionBD.miConexion);
                comando.Parameters.AddWithValue("@fecha", fecha);
                comando.Parameters.AddWithValue("@servicio", servicio);
                ConexionBD.miConexion.Open();
        ...
        public static int GetCuposHora(string servicio, string fecha, string hora)
        {
            ...
            try
            {
                string Query = "SELECT Cupos FROM Pollux.fecha_disponible where Servicio= @servicio AND Fecha= @fecha AND Hora= @hora;";
                MySqlCommand comando = new MySqlCommand(Query, ConexionBD.miConexion);
                comando.Parameters.AddWithValue("@servicio", servicio);
                comando.Parameters.AddWithValue("@fecha", fecha);
        ...
        public static bool CargarHoras(ComboBox combo, string servicio, string fecha, ArrayList horasNoDisponibles, ArrayList horasNoDisponibles2, string ida, string vuelta, int ci,int nro)
        {
            ...
            try
            {
                string Query = "SELECT Hora FROM Pollux.fecha_disponible WHERE Servicio=@servicio  and Fecha= @fecha;";
        ...
        public static ArrayList CargarHoras2(string servicio, string fecha)
        {
            ...
            try
            {
                string Query = "SELECT Hora FROM Pollux.fecha_disponible WHERE Servicio=@servicio  and Fecha= @fecha and (Cupos=-1 or Cupos > 0);";
                MySqlConnection miConexion = new MySqlConnection(ConexionBD.conexion);
                MySqlCommand comando = new MySqlCommand(Query, miConexion);
                comando.Parameters.AddWithValue("@servicio", servicio);
                comando.Parameters.AddWithValue("@fecha", fecha);
        ...
        public static bool MostrarTablaCupos(DataGridView tabla, DateTime fecha, string servicio)
        {
            try
            {
                string Query = "SELECT fd.Hora, fd.Cupos 'Cupos restantes',s.CuposMax 'Cupos maximos'FROM Pollux.fecha_disponible fd join Pollux.servicio s on fd.Servicio=s.Nombre where fd.Fecha=@fecha and fd.Servicio=@servicio order by fd.hora;";
                MySqlCommand comando = new MySqlCommand(Query, ConexionBD.miConexion);
            ...
            {
                ConexionBD.miConexion.Close();
                MessageBox.Show("MostrarTablaCupos: " + f.Message);
                return false;
            }
            return true;
        }
        public static DataTable MostrarTablaCuposDT(DateTime fecha, string servicio)
        {
            ...
            try
            {
                string Query = "SELECT fd.Servicio,fd.Fecha, fd.Hora, fd.Cupos 'Cupos restantes',s.CuposMax 'Cupos maximos'FROM Pollux.fecha_disponible fd join Pollux.servicio s on fd.Servicio=s.Nombre where fd.Fecha=@fecha and fd.Servicio=@servicio order by fd.hora;";
                MySqlCommand comando = new MySqlCommand(Query, ConexionBD.miConexion);
                comando.Parameters.AddWithValue("@fecha", fecha.ToString("yyyy-MM-dd"));
                comando.Parameters.AddWithValue("@servicio", servicio);
                ConexionBD.miConexion.Open();
            ...
            {
                ConexionBD.miConexion.Close();
                MessageBox.Show("MostrarTablaCuposDT: " + f.Message);
                return dTable;
            }
            return dTable;
        }
        ...
        public static bool modificarCapacidad(string servicio, string fecha, bool SumarCapacidad)
        {
            ...
            else if (Capacidad > 0)
            {
                ...
                try
                {
                    ...
                    string Query = "UPDATE Pollux.capacidad SET CapacidadActual =@Capacidad WHERE Servicio=@servicio and Fecha=@fecha;";
                    MySqlCommand comando = new MySqlCommand(Query, ConexionBD.miConexion);
        ...
        public static bool ModificarCupos(string fecha, string hora, string servicio, bool SumarCupo)
        {
            ...
            else if (cupos > 0)
            {
                ...
                try
                {
                    ...
                    string Query = "UPDATE Pollux.fecha_disponible SET Cupos=@cupos WHERE Servicio=@servicio and Fecha= @fecha and Hora=@hora;";
                    MySqlCommand comando = new MySqlCommand(Query, ConexionBD.miConexion);
